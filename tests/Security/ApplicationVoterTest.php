...
class ApplicationVoterTest extends TestCase
{
    ...
    public function testRecruiterCanHireForOwnOfferApplication(): void
    {
        $recruiter = (new User())->setEmail('recruiter@example.com')->setAccountType(User::ACCOUNT_TYPE_COMPANY);
        $this->setId($recruiter, 10);
        ...
        self::assertSame(VoterInterface::ACCESS_GRANTED, $voter->vote($token, $application, [ApplicationVoter::HIRE]));
    }

    public function testCandidateCannotHireApplication(): void
    {
        $candidate = (new User())->setEmail('candidate@example.com')->setAccountType(User::ACCOUNT_TYPE_PERSON);
        $this->setId($candidate, 42);
        ...
        self::assertSame(VoterInterface::ACCESS_DENIED, $voter->vote($token, $application, [ApplicationVoter::HIRE]));
    }
